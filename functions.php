...
function validateDate($value, $format = 'Y-m-d') {
    date_default_timezone_set("Europe/Moscow");
    if($value) {
        if(!date_create_from_format($value, $format)) return "Содержимое поля «дата завершения» должно быть датой в формате «ГГГГ-ММ-ДД»";
        if(strtotime('now') < strtotime($value)) return "Эта дата должна быть позднее или равна текущей";
    }
    else return null; 
};