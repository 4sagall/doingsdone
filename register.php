...
if (!$link) {
    ...
    /** @var array $users */
    $page_content = include_template('register-form.php', [
        'users' => $users,
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {   //проверяем суперглобальную переменную SERVER на то какой метод отправки был использован, в данном случае 'POST', согласно форме  
    ...
    $new_user = filter_input_array(INPUT_POST, options: ['email' => FILTER_DEFAULT, 'password' => FILTER_DEFAULT, 'name' => FILTER_DEFAULT]); //передаем в переменную $new_user значения полей из формы
